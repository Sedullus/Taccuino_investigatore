...
Il prototipo di design (`docs/design/`) usa Archivo Narrow, IBM Plex Mono e Source Serif 4 da Google Fonts. **L'app pubblicata non li carica**: caricarli da Google Fonts a runtime sarebbe una chiamata di rete esterna, non compatibile con il vincolo "nessuna chiamata di rete a runtime, a parte il caricamento dell'app". I token `--font-ui`, `--font-cifre`, `--font-prosa` (`src/theme/tokens.css`) indicano quei nomi con una lista di fallback (`system-ui`, `ui-monospace`, `Georgia, serif`): il browser usa quindi i font di sistema. Per usare davvero quei font senza chiamate di rete servirebbe includerne i file (woff2, licenza SIL Open Font License per tutti e tre) nel repository e servirli da `public/`.
